...
def test_cca_consistency(matrices2):
    ...
    assert torch.testing.assert_allclose(cca_svd[0].abs(), cca_qr[0].abs()), f"a: {cca_svd[0].abs()}!={cca_qr[0].abs()}"
    assert torch.testing.assert_allclose(cca_svd[1].abs(), cca_qr[1].abs()), f"b: {cca_svd[1].abs()}!={cca_qr[1].abs()}"
    assert torch.testing.assert_allclose(cca_svd[2], cca_qr[2]), f"diag: {cca_svd[2].abs()}!={cca_qr[2].abs()}"
