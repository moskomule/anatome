...
def test_cca_consistency(matrices2):
    ...
    assert torch.testing.assert_close(cca_svd[0].abs(), cca_qr[0].abs(), atol=1e-3, rtol=0), \
        f"a: {cca_svd[0].abs()}!={cca_qr[0].abs()}"
    assert torch.testing.assert_close(cca_svd[1].abs(), cca_qr[1].abs(), atol=1e-3, rtol=0), \
