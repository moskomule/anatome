...
def test_cca_consistency(matrices2):
    ...
    assert torch.testing.assert_close(cca_svd[0].abs(), cca_qr[0].abs()), f"a: {cca_svd[0]}!={cca_qr[0]}"
    assert torch.testing.assert_close(cca_svd[1].abs(), cca_qr[1].abs()), f"b: {cca_svd[1]}!={cca_qr[1]}"
    assert torch.testing.assert_close(cca_svd[2], cca_qr[2]), f"diag: {cca_svd[2]}!={cca_qr[2]}"
